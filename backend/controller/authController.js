...
module.exports.userRegister = (req, res) => {
    const form = formidable();
     form.parse(req, async (err, fields, files) => {
       ...
       if(!email){
            error.push('Please provide your Email ID');
       }
       ...
       if(!confirmPassword){
            error.push('Please fill the Confirm Password field');
       }
       if(password && confirmPassword && password !== confirmPassword){
            error.push('Your Password and Confirm Password fields are not matching');
       }
       if(password && password.length < 6){
            error.push('Password should must be of 6 Characters');
       }
